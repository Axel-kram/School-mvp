import { supabase, GradeGoal, StudentProfile, StudySession, Exam, StudyAlarm, TeacherProfile, SchoolClass, TeacherMaterial } from './supabase'
...
export const profile = {
  ...
  // Koppla eleven till en klass via lärarens 6-teckens join-kod.
  // Krävs för att lärarens material och klasstillhörighet ska fungera.
  joinClass: async (joinCode: string) => {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) throw new Error('Not authenticated')
    const { data: cls, error: findErr } = await supabase
      ...
      .single()
    if (findErr || !cls) throw new Error('Ingen klass hittades med den koden')
    const { error } = await supabase
      .from('student_profiles')
      .upsert({ user_id: user.id, class_id: cls.id, updated_at: new Date().toISOString() })
    if (error) throw error
  },

  // Spara elevens kontakt-e-post + telefon för larm/notiser. Separat opt-in,
  // krävs inte för att använda resten av appen (GDPR: separat samtycke).
  setNotifyContact: async (notifyEmail: string | null, phoneNumber: string | null) => {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) throw new Error('Not authenticated')
    const { error } = await supabase
      .from('student_profiles')
      .upsert({ user_id: user.id, notify_email: notifyEmail, phone_number: phoneNumber, updated_at: new Date().toISOString() })
    if (error) throw error
  },
...
// ─── Lärare ────────────────────────────────────────────────────────────────────
// Helt ny del: läraren loggar in, skapar klasser, laddar upp material.
// Materialet gör att AI-genereringen (quiz/flashcards/skriv/prov/chatt) kan
// utgå från exakt det klassen jobbar med, istället för bara Skolverkets
// kursplaner i allmänhet.

export const teacher = {
  getProfile: async (): Promise<TeacherProfile | null> => {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return null
    const { data } = await supabase
  ...
  // Skapas automatiskt första gången en lärare loggar in om profilen inte redan finns
  ensureProfile: async (fullName?: string, schoolName?: string): Promise<TeacherProfile> => {
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) throw new Error('Not authenticated')
    const { data, error } = await supabase
      .from('teacher_profiles')
      .upsert({ user_id: user.id, full_name: fullName, school_name: schoolName, updated_at: new Date().toISOString() })
      .select('*')
      .single()
    if (error) throw error
    return data
...
export const classes = {
  getMine: async (): Promise<SchoolClass[]> => {
    const teacherProfile = await teacher.getProfile()
  ...
  create: async (name: string, subject: string): Promise<SchoolClass> => {
    const teacherProfile = await teacher.getProfile() ?? await teacher.ensureProfile()
    const { data, error } = await supabase
      .from('classes')
      .insert({ teacher_id: teacherProfile.id, name, subject })
      .select('*')
      .single()
    if (error) throw error
    return data
...
export const material = {
  getForClass: async (classId: string): Promise<TeacherMaterial[]> => {
    const { data } = await supabase
  ...
  // Steg 1: skapa en rad med status "processing", innan filen är klarbearbetad.
  // Steg 2: ladda upp filen till Supabase Storage (bucket: "teacher-materials").
  // Steg 3: be n8n extrahera texten (PDF→text) och spara den i extracted_text.
  upload: async (classId: string, title: string, file: File): Promise<TeacherMaterial> => {
    const teacherProfile = await teacher.getProfile() ?? await teacher.ensureProfile()
    ...
    // Trigga textextraktion i n8n. Vi väntar inte på svaret här — frontend
    // pollar/refetchar materiallistan, eller får statusen via realtids-subscription.
    const isPlainText = file.type === 'text/plain'
    callN8N('extract-material', {
      material_id: row.id,
      file_path: isPlainText ? null : filePath,
      raw_text: isPlainText ? await file.text() : null,
    }).catch(() => {
      // Om n8n-anropet misslyckas direkt sätter vi status till failed manuellt
      supabase.from('teacher_materials').update({ status: 'failed' }).eq('id', row.id)
    })
